...
The `groovy/liquibase/harness/compatibility/advanced/AdvancedTest.groovy` test executes changelogs against the database, executes generateChangelog and
diffChangelog commands, validates the SQL generated by them as well as whether they make the expected changes.

* The test behavior is as follows:
    * The test reads the changesets from the changelogs provided in `src/main/resources/liquibase/harness/compatibility/advanced/changelogs` folders (recursively)
    * Runs Liquibase 'updateSql' command to generate query
    * Compares generated query with expected query provided in `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/setup`
    ...
    * For all modeled changelog formats the test verifies generated changelog contains required changeset, that should be the same as initial changelog name (if initial changelog was `createTable.xml`, the test will search for createTable changeset)
    * For sql format the test compares query from generated changelog with expected query (provided in `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/generateChangelog`)
    * If validation was successful, the test then runs `liquibase updateSql` on generated changelog to generate query
    * Compares generated query with expected query (provided in `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/generateChangelog`)
    ...
    * The test then runs `liquibase update` with changelog, provided in `src/main/resources/liquibase/harness/compatibility/advanced/secondaryInstanceChangelogs` folder
    ...
    * For sql format the test compares query from generated changelog with expected query (provided in `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/diffChangelog`)
    * If validation was successful, the test then runs `liquibase updateSql` on generated changelog to generate query
    * Compares generated query with expected query (provided in `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/diffChangelog`)
    * If query is correct, the test then runs `liquibase update` to deploy diff changelog to secondary DB instance
    * The test runs `liquibase diff` command referencing secondary DB instance
    * Compares generated diff with expected diff, provided in `src/main/resources/liquibase/harness/compatibility/advanced/expectedDiff/empty.txt` to verify no differences between two instances
    * Deployed changes are then rolled back from primary DB instance by either using `rollbackToDate` (default) or `rollback` by tag (**test-harness-tag**). See `-DrollbackStrategy` option below for more information.
    * The test then clears secondary DB instance by running `liquibase dropAll`
...
2) Go to `src/main/resources/liquibase/harness/compatibility/advanced/secondaryInstanceChangelogs` and add the xml changelog to setup secondary DB instance for diffChangelog command verification.
- Configure this changelog to contain different version same change type as initial changelog, so that diffChangelog command generates changesets for both creating and dropping object type under test
- You will need to add this under the database specific folder.
- If you would like to test another DB type, please add the requisite folder.
2) Go to `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/setup` and add expected query for initial changelog validation.
- You will need to add this under the database specific folder.
...
- If you would like to test another DB type, please add the requisite folder.
3) Go to `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/generateChangelog` and add expected query for generate changelog validation.
- You will need to add this under the database specific folder.
- If you would like to test another DB type, please add the requisite folder.
4) Go to `src/main/resources/liquibase/harness/compatibility/advanced/verificationSql/diffChangelog` and add expected query for diff changelog validation.
- You will need to add this under the database specific folder.
- If you would like to test another DB type, please add the requisite folder.
