...
* The test behavior is as follows:
    * It reads the changesets from the changelogs provided in `src/main/resources/liquibase/harness/compatibility/advanced/changelogs` folders (recursively)
    ...
    * For not sql formats the test verifies generated changelog contains required changeset, that should be the same as initial changelog name (if initial changelog was `createTable.xml`, the test will search for createTable changeset)
    ...
    * For not sql formats the test verifies generated changelog contains required changeset, that should be the same as initial changelog name, as well as reversed changeset (if initial changelog was `createTable.xml`, the test will search for createTable and dropTable changesets)
