...
- Files directly in that root apply to all databases. Example: `liquibase/harness/changes/changelogs`
- Files in a subdirectory named for the database type apply to only that type of database. Example: `liquibase/harness/changes/changelogs/mysql`
- Files in a subdirectory with a major version apply only to the major version of the database. Example: `liquibase/harness/changes/changelogs/mysql/8`
- Files in a subdirectory with a minor version apply only to the minor version of the database. Example: `liquibase/harness/changes/changelogs/mysql/8/0`
...
The `groovy/liquibase/harness/ChangeObjectsTest.groovy` test executes changelogs against the database and validates the SQL generated by them as well as 
whether they make the expected changes.

* The test behavior is as follows:
  * It reads the changesets from the changelogs provided in `src/main/resources/liquibase/harness/changelogs` folders (recursively)
  * Runs the changeset thru the SqlGeneratorFactory to generate SQL
  * Compares the generated SQL with the expected SQL (provided in `src/main/resources/liquibase/harness/expectedSql`)
  ...
  * Finally, the actual DB snapshot is compared to the expected DB snapshot (provided in `src/main/resources/liquibase/harness/expectedSnapshot`)
...
1) Go to `src/main/resources/liquibase/harness/changelogs` and add the xml (or other) changeset for the change type you
 want to test.
...
2) Go to `src/main/resources/liquibase/harness/expectedSql` and add the expected generated SQL. 
...
3) Go to `src/main/resources/liquibase/harness/expectedSnapshot` and add the expected DB Snapshot results.
...
* `-DchangeObjects=createTable,dropTable` flag allows you to override changeObjects configured in `harness-config.yml`. Use comma
 separated lists.
