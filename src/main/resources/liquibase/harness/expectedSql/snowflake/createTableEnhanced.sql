...
CREATE TABLE LTHDB.TESTHARNESS.TEST_TRANSIENT_TABLE (id INT NOT NULL, name VARCHAR(100), created_date timestamp DEFAULT current_timestamp::timestamp_ntz, CONSTRAINT PK_TEST_TRANSIENT_TABLE PRIMARY KEY (id)) TRANSIENT COMMENT = 'Transient test table'
COMMENT ON TABLE LTHDB.TESTHARNESS.TEST_TRANSIENT_TABLE IS 'Transient test table'
...
COMMENT ON TABLE LTHDB.TESTHARNESS.TEST_CLUSTERED_TABLE IS 'Table with clustering keys'
...
COMMENT ON TABLE LTHDB.TESTHARNESS.TEST_RETENTION_TABLE IS 'Table with data retention'
...
CREATE TABLE LTHDB.TESTHARNESS.TEST_FULL_FEATURE_TABLE (id INT NOT NULL, category VARCHAR(50), data VARCHAR(500), created_date timestamp DEFAULT current_timestamp::timestamp_ntz, CONSTRAINT PK_TEST_FULL_FEATURE_TABLE PRIMARY KEY (id)) TRANSIENT CLUSTER BY (category) DATA_RETENTION_TIME_IN_DAYS = 1 COMMENT = 'Full featured Snowflake table using new change type'
COMMENT ON TABLE LTHDB.TESTHARNESS.TEST_FULL_FEATURE_TABLE IS 'Full featured Snowflake table using new change type'
