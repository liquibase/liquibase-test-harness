...
case $db in
  ...
  "diff")
    docker compose up -d postgres-17 postgres-16 postgres-14 postgres-13 mysql-5.7 mysql-5.6 mysql-8 mysql-8.4 mariadb-10.4 mariadb-10.5 mariadb-10.6 mariadb-10.7 mssql-2019 mssql-2022
    sleep 40
    docker ps -a
    ;;

  "mssql-2017")
    docker compose up -d mssql-2017
    sleep 60
    docker ps -a
  ...
  "hsqldb-2.5"|"hsqldb-2.6"|"hsqldb-2.7"|"firebird-3"|"firebird-4")
    docker compose up -d $db
    sleep 20
    exit 0
    ;;
